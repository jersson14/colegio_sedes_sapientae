...
var tbl_ingresos_diversos;
function listar_ingresos_diversos(){
    tbl_ingresos_diversos = $("#tabla_ingresos_diversos").DataTable({
      "ordering":false,   
      "bLengthChange":true,
      ...
      "processing": true,
      "ajax":{
          "url":"../controller/ingresos/controlador_listar_ingresos_diversos.php",
          type:'POST'
      },
      dom: 'Bfrtip', 
        ...
        
    {
      ...
      filename: function() {
        return  "LISTA DE INGRESOS DIVERSOS"
      },
        title: function() {
          return  "LISTA DE INGRESOS DIVERSOS" }
...
tbl_ingresos_diversos.on('draw.td',function(){
  var PageInfo = $("#tabla_ingresos_diversos").DataTable().page.info();
  tbl_ingresos_diversos.column(0, {page: 'current'}).nodes().each(function(cell, i){
    cell.innerHTML = i + 1 + PageInfo.start;
  });
});
}

function listar_ingresos_diversos_filtro(){
  ...
  tbl_ingresos_diversos = $("#tabla_ingresos_diversos").DataTable({
    ...
    "processing": true,
    "ajax":{
        "url":"../controller/ingresos/controlador_listar_ingresos_diversos_filtros.php",
        type:'POST',
        data:{
          indi:indi,
          fechaini:fechaini,
  ...
  title: function() {
    ...
    {"data":"estado",
      render: function(data,type,row){
              if(data=='VALIDO'){
              return "<button class='editar btn btn-primary  btn-sm' title='Editar datos de ingreso'><i class='fa fa-edit'></i> Editar</button>&nbsp;&nbsp; <button class='delete btn btn-danger  btn-sm' title='Anular ingreso'><i class='fa fa-trash'></i> Anular</button>";
              }else{
                return "<button hidden class='editar btn btn-primary  btn-sm' title='Editar datos de ingreso'><i class='fa fa-edit'></i> Editar</button>&nbsp;&nbsp; <button hidden class='delete btn btn-danger  btn-sm' title='Anular ingreso'><i class='fa fa-trash'></i> Anular</button>&nbsp;&nbsp; <button class='view btn btn-warning  btn-sm' title='Motivo de anulación'><i class='fa fa-eye'></i> Ver motivo de anulación</button>";
              }
      }   
  },
...
function Cargar_Indicadores(){
    $.ajax({
      "url":"../controller/ingresos/controlador_cargar_select_indicadores.php",
      type:'POST',
    }).done(function(resp){
...
$('#tabla_ingresos_diversos').on('click','.editar',function(){
  var data = tbl_ingresos_diversos.row($(this).parents('tr')).data();

  if(tbl_ingresos_diversos.row(this).child.isShown()){
      var data = tbl_ingresos_diversos.row(this).data();
  }
  $("#modal_editar").modal('show');
  document.getElementById('txt_id_ingreso').value=data.id_ingreso;
...
function Registrar_ingreso() {
  ...
  $.ajax({
    url: "../controller/ingresos/controlador_registrar_ingresos.php",
    type: 'POST',
    data: {
      indi: indi,
      cantidad: cantidad,
    ...
    if (resp > 0) {
      Swal.fire("Mensaje de Confirmación", "¡Nuevo ingreso registrado satisfactoriamente!", "success").then((value) => {
...
function Modificar_Ingreso(){
  let id = document.getElementById('txt_id_ingreso').value;
  ...
  $.ajax({
    "url":"../controller/ingresos/controlador_modificar_ingreso.php",
    type:'POST',
    data:{
      id:id,
      indi:indi,
...
//ELIMINANDO ROL
$('#tabla_ingresos_diversos').on('click','.delete',function(){
  var data = tbl_ingresos_diversos.row($(this).parents('tr')).data();
  ...
  document.getElementById('txt_id_ingreso_anular').value=data.id_ingreso;
...
function Anular_Ingreso(){
  let id = document.getElementById('txt_id_ingreso_anular').value;
...
$('#tabla_ingresos_diversos').on('click','.view',function(){
  var data = tbl_ingresos_diversos.row($(this).parents('tr')).data();
...
var tbl_ingresos_pensiones;
function listar_ingresos_pensiones(){
  tbl_ingresos_pensiones = $("#tabla_ingresos_pensiones").DataTable({
      "ordering":false,   
      "bLengthChange":true,
      "searching": { "regex": false },
      "lengthMenu": [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] ],
      ...
      "processing": true,
      "ajax":{
          "url":"../controller/ingresos/controlador_listar_ingresos_pensiones.php",
          type:'POST'
      },
      dom: 'Bfrtip', 
...
tbl_ingresos_pensiones.on('draw.td',function(){
  var PageInfo = $("#tabla_ingresos_pensiones").DataTable().page.info();
  tbl_ingresos_pensiones.column(0, {page: 'current'}).nodes().each(function(cell, i){
    cell.innerHTML = i + 1 + PageInfo.start;
  });
});
}

function listar_ingresos_pensiones_filtro(){
  let fechaini = document.getElementById('txtfechainicio2').value;
  let fechafin = document.getElementById('txtfechafin2').value;
  ...
  tbl_ingresos_pensiones = $("#tabla_ingresos_pensiones").DataTable({
    ...
    "processing": true,
    "ajax":{
        "url":"../controller/ingresos/controlador_listar_ingresos_pensiones_filtros.php",
        type:'POST',
        data:{
          fechaini:fechaini,
          fechafin:fechafin
